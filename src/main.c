...
void main(void)
{
  ...
  SysTick_Config(SystemCoreClock / 1000000);                                    // Включаем системный таймер, с прерыванием на 1 мкс
  
  //Инициализируем EtherCAT 
  EtherCATDevice.Sync = ASYNC;
  EtherCATDevice.curState = NOT_INIT;
  EtherCAT_Init(NO_RESET, &EtherCATDevice);
  ...
  EtherCAT_MainTask();                                                          // EtherCAT routine
  EtherCATDevice.BufferIn.Byte[0] = us_Tick >> 6;                               // Send some trash to LAN
